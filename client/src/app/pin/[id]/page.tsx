...
const AuthorInfo = ({ author }: { author: Author }) => (
  <div className="group flex items-center gap-4 p-4 rounded-2xl bg-gradient-to-r from-white/80 to-white/60 backdrop-blur-sm border border-white/30 hover:from-white/90 hover:to-white/80 transition-all duration-300 hover:shadow-xl hover:shadow-orange-500/10">
    ...
    <div>
      ...
      <p className="text-sm font-medium bg-gradient-to-r from-orange-600 to-amber-600 bg-clip-text text-transparent">
        ✨ Pin Creator
      </p>
    </div>
  </div>
...
const EditPinModal = ({
  ...
  onClose: () => void;
  pin: PinDetails;
  onSave: (updatedData: { title: string; externallink: string }) => void;
}) => {
  const [title, setTitle] = useState(pin.title);
  const [externallink, setExternallink] = useState(pin.externallink);
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full">
        <h2 className="text-2xl font-bold text-slate-800 mb-6">Edit Pin</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-4 pt-4">
            ...
            <button
              type="submit"
              className="px-6 py-2 rounded-lg text-white bg-orange-500 hover:bg-orange-600 transition-colors font-semibold"
            >
              Save Changes
            </button>
          </div>
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full text-center">
        ...
        <p className="text-slate-600 mb-8">
          This action cannot be undone. You will permanently delete this pin.
        </p>
        <div className="flex justify-center gap-4">
          <button
            onClick={onClose}
            className="px-6 py-2 rounded-lg text-slate-600 bg-slate-100 hover:bg-slate-200 transition-colors font-semibold"
          >
            Cancel
          </button>
          <button
            onClick={onConfirm}
            className="px-6 py-2 rounded-lg text-white bg-red-600 hover:bg-red-700 transition-colors font-semibold"
          >
            Delete
          </button>
        </div>
...
export default function PinPage() {
  ...
  useEffect(() => {
    // --- NEW: Function to decode JWT from localStorage ---
    const decodeToken = () => {
  ...
  // --- NEW: Handle Pin Deletion ---
  const handleDeletePin = async () => {
    if (!pin) return;
    const token = localStorage.getItem("authToken");

    try {
      const response = await fetch(`${baseUrl}/api/pins/${pin.id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      // On successful deletion, close modal and redirect
      setIsDeleteModalOpen(false);
      router.push("/home"); // or wherever your main gallery is
    } catch (err: any) {
  ...
  const handleSaveEdit = async (updatedData: {
    ...
    try {
      ...
      const savedPin: PinDetails = await response.json();

      // Update the state with the new pin details
      setPin((prevPin) => ({ ...prevPin!, ...savedPin }));
  ...
  const isAuthor = currentUser && currentUser.userId === pin.author.id;
